        ...
        if (typeof options == 'string') {
            ...
            selector.each(function(){
                ...
                if (options == 'getTags')
                    ...
                    // insert new tag
                    $('<li>'+o.delimiter[0]+'<div class="tag-editor-tag"></div><div class="tag-editor-delete"><i></i></div></li>').appendTo(ed).find('.tag-editor-tag')
                        .html('<input type="text" maxlength="'+o.maxLength+'">').addClass('newtag').find('input').val(val).blur();
...
*/
        ...
        return selector.each(function(){
            ...
            // markup for new tag
            var new_tag = '<li>'+o.delimiter[0]+'<div class="tag-editor-tag"></div><div class="tag-editor-delete"><i></i></div></li>';
//            var new_tag = '<li><div class="tag-editor-spacer">&nbsp;'+o.delimiter[0]+'</div><div class="tag-editor-tag"></div><div class="tag-editor-delete"><i></i></div></li>';
            ...
            function update_globals(init){
                ...
                //여기 넘겨주면됨
                console.log('수정된 해시태그 :'+(tag_list.length ? tag_list.join(',') : null));
                $('#board_hash').val((tag_list.length ? tag_list.join(',') : null));
                //
            }
            ...
            function split_cleanup(input){
                ...
                for (var i=0; i<sub_tags.length; i++) {
                    ...
                    old_tags.push(tag);
                    li.before('<li>'+o.delimiter[0]+'<div class="tag-editor-tag">'+escape(tag)+'</div><div class="tag-editor-delete"><i></i></div></li>');
//                    li.before('<li><div class="tag-editor-spacer">&nbsp;'+o.delimiter[0]+'</div><div class="tag-editor-tag">'+escape(tag)+'</div><div class="tag-editor-delete"><i></i></div></li>');
            ...
            for (var i=0; i<tags.length; i++) {
                ...
                if (tag) {
                    if (o.forceLowercase) tag = tag.toLowerCase();
                    tag_list.push(tag);
                    ed.append('<li>'+o.delimiter[0]+'<div class="tag-editor-tag">'+escape(tag)+'</div><div class="tag-editor-delete"><i></i></div></li>');
//                    ed.append('<li><div class="tag-editor-spacer">&nbsp;'+o.delimiter[0]+'</div><div class="tag-editor-tag">'+escape(tag)+'</div><div class="tag-editor-delete"><i></i></div></li>');
            ...
            //
            console.log('기존 게시물 해시태그 : '+(tags.length ? tags.join(',') : '----'));
            //
            
            // init sortable
            if (o.sortable && $.fn.sortable) ed.sortable({
                distance: 5, cancel: ' input', helper: 'clone',
